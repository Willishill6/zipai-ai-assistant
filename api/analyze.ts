...
// ===== V10.0 识别 Prompt =====
// 字牌标准模版图片URL（用于识别参考）
const TILE_TEMPLATE_URL = 'https://files.manuscdn.com/user_upload_by_module/session_file/310519663444141745/RuIMhuClHtjVtzgv.png';
...
## 颜色规则（最关键！）
- ❤️ 红色牌只有这6种：二、七、十（小字）和贰、柒、拾（大字）
- ⬛ 黑色牌是其余所有：一三四五六八九（小字）和壹叁肆伍陆捌玖（大字）和鬼

## 大字 vs 小字区分
| 数 | 小字 | 大字 | 颜色 | 小字特征 | 大字特征 |
|-----|------|------|------|------|------|
| 1 | 一 | 壹 | 黑 | 一横，极简 | 有士冒豆，复杂 |
| 2 | 二 | 贰 | 红 | 两横，极简 | 有尋和贝，复杂 |
| 3 | 三 | 叁 | 黑 | 三横，极简 | 有厶和大，复杂 |
| 4 | 四 | 肆 | 黑 | 口中两竖 | 有聿和长横 |
| 5 | 五 | 伍 | 黑 | 横竖横竖，无偏旁 | 左有人旁仹 |
| 6 | 六 | 陆 | 黑 | 点横撇点，无偏旁 | 左有耳刀旁阝 |
| 7 | 七 | 柒 | 红 | 一横一弯，极简 | 上木下复杂 |
| 8 | 八 | 捌 | 黑 | 撇捣，极简 | 左有提手旁扬 |
| 9 | 九 | 玖 | 黑 | 撇和弯钉，极简 | 左王右久 |
| 10 | 十 | 拾 | 红 | 十字形，极简 | 左有提手旁扬 |

## 手牌区域识别方法
- 手牌在截图底部，展示为两排或三排（取决于手牌多少）
- 两排：上排通幼10-11张，下排通幼10-11张，共20-21张
- 三排：每排大约7-8张，共还20-21张
- 底部左侧的明牌组（碰/嵌组）不是手牌！
- 有明牌组时手牌会少于20张

## ❗ 必须遵守
1. 只能使用上述21个合法字，绝对不能用其他汉字
2. 每种牌最多4张，鬼牌最多2张
3. 红色牌只有：二、七、十、贰、柒、拾，其他必须是黑色
4. 识别完后必须自我检查：有没有非法字？有没有超限？`;
    ...
    [
      { role: "system", content: TILE_RECOGNITION_PROMPT },
      {
        ...
        content: [
          {
            type: "text",
            text: `下面是桂林飞飞字牌的标准牌面图（共21种），请仔细记住每种牌的字形和颜色：`,
          },
          {
            type: "image_url",
            image_url: { url: TILE_TEMPLATE_URL, detail: "high" },
          },
          {
            type: "text",
            text: `现在请识别下面这张游戏截图中的手牌。

识别规则：
1. 手牌在截图底部，展示为两排或三排（取决于手牌多少）
2. 底部左侧的明牌组（碰/嵌）不是手牌！
3. 每张牌先对照模版图确认颜色（红/黑），再对照字形判断大小字
4. 红色牌只有：二、七、十、贰、柒、拾
5. 每种牌最多4张，鬼牌最多2张
        ...
        [
          { role: "system", content: TILE_RECOGNITION_PROMPT },
          {
            ...
            content: [
              ...
              {
                type: "text",
                text: `上次识别只找到${handTiles.length}张手牌，这不对！手牌应该20或21张。

请重新仔细识别下面截图中的手牌。手牌在截图底部，可能是两排或三排。每张牌对照模版图识别。总数必须20或21张！`,
              },
              {
                type: "image_url",
                image_url: { url: imageBase64, detail: "high" },
